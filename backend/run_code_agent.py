...
def create_app() -> FastAPI:
    ...
    redis_repo = RedisRepository(redis_url, prefix="code_agent")
    ...
    @app.get("/health")
    async def health():
        return {"status": "healthy", "agent": "code"}

    @app.post("/execute", response_model=TaskResponse)
    async def execute_task(request: TaskRequest):
        """Execute a code generation or analysis task"""
        try:
...
async def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description="Run Code Agent")
    parser.add_argument("--port", type=int, default=8002, help="Port to run on")
    parser.add_argument("--host", type=str, default="0.0.0.0", help="Host to bind to")
    args = parser.parse_args()

    logger.info(f"Starting Code Agent on {args.host}:{args.port}")
